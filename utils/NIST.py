...
# wxRays (C) 2014-2016 Serhii Lysovenko
#
...
def dump_card(card):
    if not card.reflexes:
        return
    elind = card.get_formula_elements()
    if elind is None:
        return
    num = card.number
    for i in elind:
        print("INSERT INTO elements_ind VALUES(%d, %d);" % (num, i))
    for i in card.reflexes:
        if len(i) == 2:
